...
public class PlaybackFragment extends SeekModePlaybackFragment implements PlaybackView {
    ...
    // MOD: dim caption mode (swipe up to enter, double-tap to exit).
    private View mDimOverlay;
    private View mSurfaceRoot;
    private android.widget.TextView mDimClock;
    private android.widget.TextView mDimCaption;
    private boolean mDimShown;
    private boolean mDimSwipeTracking;
    private boolean mDimSwipeConsumed;
    private float mDimSwipeStartX;
    private float mDimSwipeStartY;
    private int mLastDimSwipeDirection;
    private long mLastDimSwipeTimeMs;
    private final android.os.Handler mDimHandler = new android.os.Handler(android.os.Looper.getMainLooper());
    private final java.text.SimpleDateFormat mDimClockFmt = new java.text.SimpleDateFormat("h:mm", java.util.Locale.getDefault());
    private static final long DIM_DOUBLE_SWIPE_TIMEOUT_MS = 1_200L;
    private final Runnable mDimClockTick = new Runnable() {
        @Override
        public void run() {
            if (mDimClock != null) {
                mDimClock.setText(mDimClockFmt.format(new java.util.Date()) + "  " + getBatteryText());
            }
            mDimHandler.postDelayed(this, 1000);
        }
    };
    ...
    public void onDispatchTouchEvent(MotionEvent event) {
        if (isRayNeoDirectTouchEvent(event)) {
            handleDimSwipe(event);
            return;
        }

        if (handleDimSwipe(event)) {
            return;
        }

        // While dim mode is up, the black overlay handles its own touches
        // (double-tap to exit); don't drive the underlying player controls.
        if (mDimShown) {
            return;
        }
    ...
    // ===== MOD: dim caption mode =====================

    /** Lazily bind the overlay views and wire the dim caption sink. */
    private void initDimMode() {
        if (mDimOverlay != null || getView() == null) {
            return;
        }
        mDimOverlay = getView().findViewById(R.id.dim_overlay);
        mSurfaceRoot = getView().findViewById(R.id.surface_root);
        mDimClock = getView().findViewById(R.id.dim_clock);
        mDimCaption = getView().findViewById(R.id.dim_caption);
        if (mSubtitleManager != null && mDimCaption != null) {
            mSubtitleManager.setDimCaptionView(mDimCaption);
        }
        if (mDimOverlay != null) {
            mDimOverlay.setOnTouchListener(new View.OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent e) {
                    return true; // swallow so nothing leaks to the player
                }
            });
        }
    }

    /** Track raw DOWN->MOVE displacement (GestureDetector.onFling is unreliable
     *  on the narrow temple pad) and toggle dim mode on a double vertical swipe. */
    private boolean handleDimSwipe(MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                mDimSwipeTracking = true;
                mDimSwipeConsumed = false;
                mDimSwipeStartX = event.getX();
                mDimSwipeStartY = event.getY();
                return false;
            case MotionEvent.ACTION_MOVE:
            case MotionEvent.ACTION_UP:
                if (!mDimSwipeTracking) {
                    return false;
                }
                float dx = event.getX() - mDimSwipeStartX;
                float dy = event.getY() - mDimSwipeStartY;
                float adx = Math.abs(dx);
                float ady = Math.abs(dy);
                if (!mDimSwipeConsumed && ady >= 22f && ady > adx * 1.15f) {
                    mDimSwipeConsumed = true;
                    handleDimModeSwipe(dy < 0f ? -1 : 1);
                    return true;
                }
                if (event.getActionMasked() == MotionEvent.ACTION_UP) {
                    mDimSwipeTracking = false;
                }
                return mDimSwipeConsumed;
            default:
                return false;
        }
    }

    private void handleDimModeSwipe(int direction) {
        long now = android.os.SystemClock.uptimeMillis();
        boolean isDoubleSwipe = direction == mLastDimSwipeDirection
                && now - mLastDimSwipeTimeMs <= DIM_DOUBLE_SWIPE_TIMEOUT_MS;

        mLastDimSwipeDirection = direction;
        mLastDimSwipeTimeMs = now;

        if (!isDoubleSwipe) {
            return;
        }

        mLastDimSwipeDirection = 0;
        mLastDimSwipeTimeMs = 0;

        if (direction < 0 && !mDimShown) {
            showDimMode();
        } else if (direction > 0 && mDimShown) {
            hideDimMode();
        }
    }

    private void showDimMode() {
        initDimMode();
        if (mDimOverlay == null || mDimShown) {
            return;
        }
        mDimShown = true;
        ...
        // Hide the video surface (a view-on-top can't occlude a SurfaceView);
        // audio keeps playing because that's independent of the surface.
        if (mSurfaceRoot != null) {
            mSurfaceRoot.setVisibility(View.INVISIBLE);
        }
        if (mDimCaption != null) {
            mDimCaption.setText("");
        }
        mDimOverlay.setVisibility(View.VISIBLE);
        mDimHandler.removeCallbacks(mDimClockTick);
        mDimHandler.post(mDimClockTick);
    }

    private void hideDimMode() {
        if (!mDimShown) {
            return;
        }
        mDimShown = false;
        ...
        if (mSurfaceRoot != null) {
            mSurfaceRoot.setVisibility(View.VISIBLE);
        }
        if (mDimOverlay != null) {
    ...
    private String getBatteryText() {
        try {
            Intent battery = getContext() != null
    ...
    private boolean isRayNeoDirectTouchEvent(MotionEvent event) {
        return RayNeoDeviceUtil.isRayNeoDevice()
    ...
    private void createSubtitleManager() {
        ...
        initDimMode(); // MOD: wire dim caption overlay to the subtitle sink
    }
