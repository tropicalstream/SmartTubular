...
public class MotherActivity extends FragmentActivity {
    ...
    private static final boolean DEBUG_RAYNEO_INPUT = true;
    ...
    /**
     * Interface for intercepting touch events before the default dispatch chain.
     * Used by RayNeoInputInterceptor to translate temple gestures to DPAD KeyEvents.
     */
    public interface TouchEventInterceptor {
        /**
         * Called before the default dispatchTouchEvent handling.
         * @return true if the event was consumed (skip default handling)
         */
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent event) {
        ...
        if (mTouchEventInterceptor != null && mTouchEventInterceptor.onInterceptTouchEvent(event)) {
            logRayNeoInput("generic motion intercepted action=" + event.getAction()
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        // Let the RayNeo interceptor handle temple touch events first
        if (mTouchEventInterceptor != null && mTouchEventInterceptor.onInterceptTouchEvent(event)) {
            logRayNeoInput("touch intercepted action=" + event.getAction()
                    + " source=0x" + Integer.toHexString(event.getSource()));
            return true;
        }
    ...
    private boolean isExternalMouse(MotionEvent ev) {
        boolean isMouseSource = (ev.getSource() & android.view.InputDevice.SOURCE_MOUSE) == android.view.InputDevice.SOURCE_MOUSE;
        boolean isMouseTool = ev.getToolType(0) == MotionEvent.TOOL_TYPE_MOUSE;
        return isMouseSource || isMouseTool;
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        logRayNeoInput("dispatchKeyEvent activity=" + getClass().getSimpleName()
                + " action=" + keyActionToString(event.getAction())
                + " key=" + KeyEvent.keyCodeToString(event.getKeyCode())
                + " source=0x" + Integer.toHexString(event.getSource())
                + " focus=" + describeCurrentFocus());
        ...
        try {
            ...
            logRayNeoInput("dispatchKeyEvent result activity=" + getClass().getSimpleName()
                    + " action=" + keyActionToString(event.getAction())
                    + " key=" + KeyEvent.keyCodeToString(event.getKeyCode())
                    + " handled=" + handled
                    + " focusAfter=" + describeCurrentFocus());
            return handled;
    ...
    private DisplayMetrics getDisplayMetrics(Context context) {
        ...
        if (sCachedDisplayMetrics == null) {
            ...
            // Centralized RayNeo/TV DPI logic
            com.liskovsoft.smartyoutubetv2.common.utils.RayNeoDeviceUtil.applyDisplayMetrics(displayMetrics, uiScale);
    ...
    /**
     * Set a touch event interceptor for translating hardware-specific input
     * (e.g., RayNeo temple gestures) into standard KeyEvents.
     */
    public void setTouchEventInterceptor(TouchEventInterceptor interceptor) {
        mTouchEventInterceptor = interceptor;
    }

    private void logRayNeoInput(String message) {
        if (DEBUG_RAYNEO_INPUT) {
            Log.d(TAG, "[RayNeoInput] %s", message);
        }
    }

    private String describeCurrentFocus() {
        try {
            android.view.View focus = getWindow() != null ? getWindow().getCurrentFocus() : null;
            if (focus == null) {
                return "null";
            }
